...
class ShadowClass {
  ...
  calculateShadowPolygon(building: typeof testdata, date?:Date) {
    ...
    codi.forEach((polygon) => {
      ...
      result.push([ this.getShadowPathBetweenTwoPolygon(polygon, shadowBase) ]);
  ...
  getShadowPathBetweenTwoPolygon(polygon1:number[][], polygon2:number[][]) {
    ...
    console.log('pairedLines', pairedLines);
  ...
  doLinesIntersect(A:number[], B:number[], C:number[], D:number[]) {
    ...
    function isBetween(P:number[], Q:number[], R:number[]) {
